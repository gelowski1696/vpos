...
export default function CylinderTypesPage(): JSX.Element {
  ...
  const importColumns: ImportColumn[] = [
    { key: 'code', label: 'Code', required: true, example: 'CYL11', aliases: ['cylinder_code', 'cyl_code'] },
    { key: 'name', label: 'Name', required: true, example: '11kg Standard Cylinder', aliases: ['cylinder_name'] },
    { key: 'sizeKg', label: 'Size (kg)', required: true, example: 11, aliases: ['size_kg', 'size'] },
    {
      key: 'depositAmount',
      label: 'Deposit Amount (Refundable)',
      required: true,
      example: 1200,
      aliases: ['deposit_amount', 'deposit']
    }
  ];
  ...
  useEffect(() => {
    ...
    codeCheckTimerRef.current = setTimeout(() => {
      ...
      void apiRequest<{ exists: boolean }>(`/master-data/cylinder-types/code-exists?${query.toString()}`)
        .then((result) => {
  ...
  return (
    <EntityManager
      allowDelete
      defaultValues={{ code: '', name: '', sizeKg: 11, depositAmount: 1200, isActive: true }}
      endpoint="/master-data/cylinder-types"
      reloadSignal={reloadSignal}
      toolbarActions={
        <MasterDataImportWizard
          title="Cylinder Types"
          entity="cylinder-types"
          endpointBase="/master-data/import/cylinder-types"
      ...
      fields={[
        ...
        { key: 'sizeKg', label: 'Size (kg)', type: 'number', required: true },
        {
          key: 'depositAmount',
          label: 'Deposit Amount (Refundable)',
          type: 'number',
          required: true,
          helperText: 'This is the customer security deposit for cylinder custody. It is tracked as a liability and refunded on cylinder return.'
        },
      ...
      renderFieldAction={({ field, disabled, setValue }) =>
        field.key === 'code' ? (
          <button
            ...
            disabled={disabled}
            onClick={() => setValue(generateShortCode('CY'))}
      ...
      transformBeforeSubmit={async (payload, context) => {
        ...
        if (normalizedCode.length < 1 || normalizedCode.length > 8) {
          throw new Error('Cylinder code must be 1 to 8 characters (A-Z, 0-9).');
        }
        const query = new URLSearchParams();
        query.set('code', normalizedCode);
        ...
        const existsResult = await apiRequest<{ exists: boolean }>(
          `/master-data/cylinder-types/code-exists?${query.toString()}`
        );
        if (existsResult.exists) {
          throw new Error(`Cylinder code "${normalizedCode}" already exists.`);
        }
        return {
          ...payload,
          code: normalizedCode
