...
export default function SalesListPage(): JSX.Element {
  ...
  const selectedCommissions = selectedDetails?.commissions ?? [];
                ...
                <div className="space-y-4">
                  ...
                  <article className="rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-800/40">
                    <div className="flex items-center justify-between gap-3">
                      <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-100">Product Commissions</h3>
                      <span className="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-700 dark:bg-slate-800 dark:text-slate-200">
                        Total {fmtMoney(selectedCommissionTotal)}
                      </span>
                    </div>
                    <div className="mt-2 overflow-x-auto">
                      <table className="min-w-[980px] text-sm">
                        <thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-600 dark:bg-slate-800/70 dark:text-slate-300">
                          <tr>
                            <th className="px-2 py-1.5">Personnel</th>
                          ...
                          {selectedCommissions.length === 0 ? (
                            <tr>
                              <td className="px-2 py-2 text-slate-500 dark:text-slate-400" colSpan={8}>
                                No product commissions recorded for this sale.
                              </td>
                            </tr>
                          ) : (
                            selectedCommissions.map((commission) => (
                              <tr className="border-t border-slate-100 dark:border-slate-800" key={commission.commission_id}>
                                <td className="px-2 py-1.5 text-slate-700 dark:text-slate-200">
                                  <p>{commission.personnel_name}</p>
                                  <p className="text-[11px] text-slate-500 dark:text-slate-400">
                                    {commission.personnel_code ?? commission.personnel_id ?? 'Snapshot'}
                                  </p>
                                </td>
                                <td className="px-2 py-1.5 text-slate-700 dark:text-slate-200">{commission.personnel_role ?? 'Personnel'}</td>
                                <td className="px-2 py-1.5 text-slate-700 dark:text-slate-200">
                                  <p>{commission.product_name}</p>
                                  <p className="text-[11px] text-slate-500 dark:text-slate-400">{commission.item_code}</p>
                                </td>
                                <td className="px-2 py-1.5 text-slate-700 dark:text-slate-200">{commission.sale_type}</td>
                                <td className="px-2 py-1.5 text-right text-slate-700 dark:text-slate-200">{fmtQty(commission.qty)}</td>
                                <td className="px-2 py-1.5 text-right text-slate-700 dark:text-slate-200">{fmtMoney(commission.commission_rate)}</td>
                                <td className="px-2 py-1.5 text-right text-slate-700 dark:text-slate-200">{fmtQty(commission.split_percent)}%</td>
                                <td className="px-2 py-1.5 text-right font-semibold text-slate-800 dark:text-slate-100">
                                  {fmtMoney(commission.commission_amount)}
                                </td>
                              </tr>
                            ))
                          )}
                    ...
                    <article className="rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-700 dark:bg-slate-800/40">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <Link
                          ...
                          href={`/delivery-dispatch?sale_id=${encodeURIComponent(selectedDetails.sale.sale_id)}` as Route}
                        >
                          Open Dispatch
                        </Link>
                      </div>
