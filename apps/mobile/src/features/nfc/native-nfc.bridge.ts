...
export async function startNativeNfcScan(): Promise<void> {
  const module = getNativeModule();
  if (!module) {
    throw new Error('Native NFC bridge is unavailable. Rebuild VPOS Android app with VposNfcBridge.');
  }
...
export function subscribeToNfcTagEvents(onTag: (tag: NfcTagEvent) => void): { remove: () => void } {
  const module = getNativeModule();
  if (!module) {
  ...
  const subscription = emitter.addListener(TAG_EVENT_NAME, (payload: unknown) => {
    ...
    const techList = Array.isArray(map.techList) ? map.techList.map((item) => String(item)) : [];
    onTag({
